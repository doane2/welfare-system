'use client'
import { useEffect, useState, useCallback } from 'react'
import api   from '../../../lib/api'
import toast from 'react-hot-toast'

type Log = {
  id: string; action: string; label: string; color: string
  entity: string; entityId: string | null
  createdAt: string
  user: { id: string; fullName: string; memberNumber: string; role: string; email: string }
}

type Stats = {
  total: number; today: number; thisWeek: number
  topActions: { action: string; label: string; count: number; color: string }[]
}

const ROLE_COLORS: Record<string, { bg: string; color: string }> = {
  SUPER_ADMIN: { bg: '#fef3c7', color: '#b45309' },
  TREASURER:   { bg: '#e0f2fe', color: '#0369a1' },
  SECRETARY:   { bg: '#f5f3ff', color: '#7c3aed' },
  MEMBER:      { bg: '#dcfce7', color: '#15803d' },
}

// ─── Official logo — raw JPEG base64 for jsPDF addImage() ────────────────────
const LOGO_BASE64 = ``

export default function AdminAuditLogsPage() {
  const [logs,         setLogs]         = useState<Log[]>([])
  const [stats,        setStats]        = useState<Stats | null>(null)
  const [actions,      setActions]      = useState<any[]>([])
  const [loading,      setLoading]      = useState(true)
  const [exportingPDF, setExportingPDF] = useState(false)
  const [page,         setPage]         = useState(1)
  const [total,        setTotal]        = useState(0)
  const [totalPages,   setTotalPages]   = useState(1)
  const [search,  setSearch]  = useState('')
  const [action,  setAction]  = useState('')
  const [from,    setFrom]    = useState('')
  const [to,      setTo]      = useState('')
  const LIMIT = 25

  const load = useCallback(async () => {
    setLoading(true)
    try {
      const params: any = { page, limit: LIMIT }
      if (search) params.search = search
      if (action) params.action = action
      if (from)   params.from   = from
      if (to)     params.to     = to
      const { data } = await api.get('/audit-logs', { params })
      setLogs(data.auditLogs || [])
      setTotal(data.total || 0)
      setTotalPages(data.totalPages || 1)
      if (data.stats) setStats(data.stats)
    } catch { toast.error('Failed to load audit logs') }
    finally  { setLoading(false) }
  }, [page, search, action, from, to])

  useEffect(() => {
    api.get('/audit-logs/actions').then(({ data }) => setActions(data.actions || [])).catch(() => {})
  }, [])

  useEffect(() => { load() }, [load])

  const resetFilters = () => { setSearch(''); setAction(''); setFrom(''); setTo(''); setPage(1) }

  // ─── Export PDF ─────────────────────────────────────────────────────────────
  const exportPDF = async () => {
    setExportingPDF(true)
    try {
      const { data } = await api.get('/audit-logs', { params: { page: 1, limit: 1000, search, action, from, to } })
      const allLogs: Log[] = data.auditLogs || []

      const { jsPDF } = await import('jspdf')
      const autoTable   = (await import('jspdf-autotable')).default
      const doc         = new jsPDF({ orientation: 'landscape', unit: 'mm', format: 'a4' })
      const pageW       = doc.internal.pageSize.getWidth()
      const pageH       = doc.internal.pageSize.getHeight()
      const margin      = 14
      const now         = new Date()

      const filterParts: string[] = []
      if (search) filterParts.push(`Search: "${search}"`)
      if (action) filterParts.push(`Action: ${action}`)
      if (from)   filterParts.push(`From: ${from}`)
      if (to)     filterParts.push(`To: ${to}`)
      const filterDesc = filterParts.length > 0 ? filterParts.join('  ·  ') : 'All records'

      // ── HEADER ────────────────────────────────────────────────────────────
      doc.setFillColor(10, 25, 52); doc.rect(0, 0, pageW, 52, 'F')
      doc.setFillColor(230, 176, 32); doc.rect(0, 0, 4, 52, 'F')
      doc.addImage(LOGO_BASE64, 'JPEG', 8, 5, 22, 22)
      doc.setTextColor(255, 255, 255); doc.setFontSize(16); doc.setFont('helvetica', 'bold')
      doc.text('CRATER SDA WELFARE SOCIETY', 35, 16)
      doc.setFontSize(8); doc.setFont('helvetica', 'normal'); doc.setTextColor(180, 200, 230)
      doc.text('Nakuru, Kenya  ·  Est. 2016  ·  System Audit Log Report', 35, 23)
      doc.setDrawColor(230, 176, 32); doc.setLineWidth(0.4); doc.line(35, 27, pageW - margin, 27)
      doc.setTextColor(245, 200, 66); doc.setFontSize(9); doc.setFont('helvetica', 'bold')
      doc.text('OFFICIAL AUDIT LOG REPORT', 35, 34)
      doc.setTextColor(160, 185, 215); doc.setFontSize(7.5); doc.setFont('helvetica', 'normal')
      doc.text(
        `Generated: ${now.toLocaleDateString('en-KE', { weekday:'long', day:'numeric', month:'long', year:'numeric' })}  ·  Filters: ${filterDesc}`,
        35, 41
      )
      doc.setFillColor(230, 176, 32); doc.roundedRect(pageW - 72, 8, 58, 16, 3, 3, 'F')
      doc.setTextColor(10, 25, 52); doc.setFontSize(7.5); doc.setFont('helvetica', 'bold')
      doc.text('TOTAL RECORDS', pageW - 43, 16, { align: 'center' })
      doc.setFontSize(9); doc.text(String(allLogs.length), pageW - 43, 22, { align: 'center' })

      // ── SUMMARY STATS STRIP ──────────────────────────────────────────────
      let y = 58
      if (stats) {
        doc.setFillColor(248, 250, 252); doc.roundedRect(margin, y, pageW - margin * 2, 24, 3, 3, 'F')
        doc.setDrawColor(210, 220, 235); doc.setLineWidth(0.25)
        doc.roundedRect(margin, y, pageW - margin * 2, 24, 3, 3, 'S')
        const kpis = [
          { label: 'Total Logs',   value: String(stats.total),    rgb: [30,58,110]  as [number,number,number] },
          { label: 'Today',        value: String(stats.today),    rgb: [21,128,61]  as [number,number,number] },
          { label: 'This Week',    value: String(stats.thisWeek), rgb: [3,105,161]  as [number,number,number] },
          { label: 'Action Types', value: String(actions.length), rgb: [180,83,9]   as [number,number,number] },
          { label: 'Exported',     value: String(allLogs.length), rgb: [30,58,110]  as [number,number,number] },
        ]
        const kpiW = (pageW - margin * 2) / kpis.length
        kpis.forEach((k, i) => {
          const kx = margin + i * kpiW
          doc.setFontSize(6.5); doc.setFont('helvetica','normal'); doc.setTextColor(120,135,155)
          doc.text(k.label.toUpperCase(), kx + kpiW / 2, y + 8, { align: 'center' })
          doc.setFontSize(11); doc.setFont('helvetica','bold'); doc.setTextColor(k.rgb[0],k.rgb[1],k.rgb[2])
          doc.text(k.value, kx + kpiW / 2, y + 18, { align: 'center' })
          if (i < kpis.length - 1) {
            doc.setDrawColor(210,220,235); doc.setLineWidth(0.2)
            doc.line(kx + kpiW, y + 4, kx + kpiW, y + 20)
          }
        })
        y += 30
      }

      // ── SECTION HEADING ──────────────────────────────────────────────────
      doc.setFontSize(8); doc.setFont('helvetica','bold'); doc.setTextColor(30,58,110)
      doc.text('AUDIT LOG ENTRIES', margin, y)
      doc.setDrawColor(230,176,32); doc.setLineWidth(0.5); doc.line(margin, y + 1.5, margin + 40, y + 1.5)

      // ── LOG TABLE ────────────────────────────────────────────────────────
      autoTable(doc, {
        startY: y + 6,
        head: [['#','Date','Time','Action','Entity','Entity ID','Performed By','Member No.','Role']],
        body: allLogs.length > 0
          ? allLogs.map((l, i) => [
              i + 1,
              new Date(l.createdAt).toLocaleDateString('en-KE', { day:'numeric', month:'short', year:'numeric' }),
              new Date(l.createdAt).toLocaleTimeString('en-KE', { hour:'2-digit', minute:'2-digit', second:'2-digit' }),
              l.label || '—', l.entity || '—',
              l.entityId ? l.entityId.slice(0,8)+'...' : '—',
              l.user?.fullName || '—',
              l.user?.memberNumber || l.user?.email || '—',
              (l.user?.role || '—').replace('_',' '),
            ])
          : [['—','No records match the selected filters','','','','','','','']],
        theme: 'grid',
        headStyles: { fillColor:[10,25,52], textColor:[255,255,255], fontSize:7.5, fontStyle:'bold', halign:'left', cellPadding:3.5 },
        bodyStyles: { fontSize:7, textColor:[51,65,85], cellPadding:3 },
        alternateRowStyles: { fillColor:[248,250,252] },
        columnStyles: {
          0: { halign:'center', fontStyle:'bold', cellWidth:8 },
          1: { fontStyle:'bold', cellWidth:26 },
          2: { cellWidth:22 },
          3: { fontStyle:'bold', textColor:[15,32,64] },
          5: { fontSize:6.5 },
          8: { halign:'center' },
        },
        margin: { left: margin, right: margin },
        didDrawCell: (hookData: any) => {
          if (hookData.section === 'body' && hookData.column.index === 8) {
            const log = allLogs[hookData.row.index]
            if (!log) return
            const rc = ROLE_COLORS[log.user?.role] || ROLE_COLORS.MEMBER
            const hex = (h:string) => [parseInt(h.slice(1,3),16), parseInt(h.slice(3,5),16), parseInt(h.slice(5,7),16)] as [number,number,number]
            const [br,bg,bb] = hex(rc.bg)
            const [tr,tg,tb] = hex(rc.color)
            doc.setFillColor(br,bg,bb)
            doc.rect(hookData.cell.x+1, hookData.cell.y+1, hookData.cell.width-2, hookData.cell.height-2, 'F')
            doc.setFontSize(6.5); doc.setFont('helvetica','bold'); doc.setTextColor(tr,tg,tb)
            doc.text(
              (log.user?.role||'').replace('_',' '),
              hookData.cell.x + hookData.cell.width/2,
              hookData.cell.y + hookData.cell.height/2 + 1,
              { align:'center' }
            )
          }
        },
      })

      // ── PER-PAGE: CONFIDENTIAL watermark + footer with logo ──────────────
      const numPages = (doc as any).internal.getNumberOfPages()
      for (let pg = 1; pg <= numPages; pg++) {
        doc.setPage(pg)
        doc.setFontSize(52); doc.setFont('helvetica','bold'); doc.setTextColor(232,235,243)
        doc.text('CONFIDENTIAL', pageW/2, pageH/2, { align:'center', angle:45 })
        doc.setDrawColor(230,176,32); doc.setLineWidth(0.5)
        doc.line(margin, pageH-18, pageW-margin, pageH-18)
        doc.setFontSize(6.5); doc.setFont('helvetica','italic'); doc.setTextColor(120,135,155)
        doc.text('CONFIDENTIAL — This report is computer-generated for internal use only.', margin, pageH-13)
        doc.setFont('helvetica','normal')
        doc.text(`Page ${pg} / ${numPages}`, pageW-margin-14, pageH-13, { align:'right' })
        doc.setFont('helvetica','bold'); doc.setTextColor(30,58,110)
        doc.text('Crater SDA Welfare Society  ·  Nakuru, Kenya  ·  Est. 2016', margin, pageH-7)
        doc.addImage(LOGO_BASE64, 'JPEG', pageW-margin-12, pageH-19, 12, 12)
      }

      doc.save(`CraterSDA_AuditLog_${now.toISOString().split('T')[0]}.pdf`)
      toast.success('PDF audit log downloaded!')
    } catch (err) {
      console.error(err); toast.error('Failed to generate PDF. Please try again.')
    } finally { setExportingPDF(false) }
  }

  const iS: React.CSSProperties = { padding:'9px 13px', borderRadius:8, border:'1.5px solid #e2e8f0', fontSize:13, outline:'none', background:'#fff' }
  const iF = (e:React.FocusEvent<HTMLInputElement|HTMLSelectElement>) => (e.target.style.borderColor='#1e3a6e')
  const iB = (e:React.FocusEvent<HTMLInputElement|HTMLSelectElement>) => (e.target.style.borderColor='#e2e8f0')

  return (
    <div style={{ padding:'32px 36px' }}>

      {/* ── Page header ───────────────────────────────────────────────────── */}
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:28 }}>
        <div>
          <h1 style={{ fontFamily:'Georgia,serif', fontSize:26, fontWeight:700, color:'#0f2040', marginBottom:4 }}>Audit Logs</h1>
          <p style={{ fontSize:14, color:'#64748b' }}>Complete record of every admin and member action in the system</p>
        </div>
        <button onClick={exportPDF} disabled={exportingPDF}
          style={{
            background: exportingPDF ? '#94a3b8' : '#0f2040',
            color:'#fff', padding:'10px 22px', borderRadius:9,
            fontSize:13, fontWeight:600, border:'none',
            cursor: exportingPDF ? 'not-allowed' : 'pointer',
            display:'flex', alignItems:'center', gap:8,
          }}>
          {exportingPDF
            ? <><span style={{ width:13, height:13, border:'2px solid rgba(255,255,255,0.3)', borderTopColor:'#fff', borderRadius:'50%', display:'inline-block', animation:'spin 0.7s linear infinite' }}/> Generating PDF...</>
            : <>📄 Export PDF</>}
        </button>
      </div>

      {/* ── Stats ─────────────────────────────────────────────────────────── */}
      {stats && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14, marginBottom:24 }}>
          {[
            { label:'Total logs',   value:stats.total,    icon:'📋', color:'#1e3a6e', bg:'#eef2ff' },
            { label:'Today',        value:stats.today,    icon:'📅', color:'#15803d', bg:'#dcfce7' },
            { label:'This week',    value:stats.thisWeek, icon:'📊', color:'#0369a1', bg:'#e0f2fe' },
            { label:'Action types', value:actions.length, icon:'🔧', color:'#b45309', bg:'#fef3c7' },
          ].map(s=>(
            <div key={s.label} style={{ background:'#fff', borderRadius:12, padding:'16px 20px', border:'1px solid #e2e8f0' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                <div style={{ fontSize:11, fontWeight:600, color:'#64748b', textTransform:'uppercase', letterSpacing:'0.07em' }}>{s.label}</div>
                <div style={{ width:30, height:30, borderRadius:8, background:s.bg, display:'flex', alignItems:'center', justifyContent:'center', fontSize:14 }}>{s.icon}</div>
              </div>
              <div style={{ fontFamily:'Georgia,serif', fontSize:26, fontWeight:700, color:s.color }}>{s.value}</div>
            </div>
          ))}
        </div>
      )}

      {/* ── Top actions breakdown ─────────────────────────────────────────── */}
      {stats?.topActions && stats.topActions.length > 0 && (
        <div style={{ background:'#fff', borderRadius:14, padding:'18px 22px', border:'1px solid #e2e8f0', marginBottom:24 }}>
          <div style={{ fontWeight:600, fontSize:14, color:'#0f2040', marginBottom:14 }}>Most frequent actions</div>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
            {stats.topActions.map(a=>(
              <button key={a.action} onClick={()=>{setAction(a.action);setPage(1)}}
                style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'5px 12px', borderRadius:99, border:`1px solid ${a.color}30`, background:`${a.color}10`, cursor:'pointer', fontSize:12, fontWeight:500, color:a.color }}>
                {a.label}
                <span style={{ background:a.color, color:'#fff', fontSize:10, fontWeight:700, padding:'1px 5px', borderRadius:99 }}>{a.count}</span>
              </button>
            ))}
          </div>
        </div>
      )}

      {/* ── Filters ───────────────────────────────────────────────────────── */}
      <div style={{ background:'#fff', borderRadius:14, padding:'16px 20px', border:'1px solid #e2e8f0', marginBottom:20 }}>
        <div style={{ display:'flex', gap:12, flexWrap:'wrap', alignItems:'flex-end' }}>
          <div style={{ flex:1, minWidth:180 }}>
            <div style={{ fontSize:11, fontWeight:600, color:'#64748b', marginBottom:5 }}>Search</div>
            <input value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}}
              placeholder="Search action, entity..."
              style={{ ...iS, width:'100%' }} onFocus={iF} onBlur={iB}/>
          </div>
          <div style={{ minWidth:180 }}>
            <div style={{ fontSize:11, fontWeight:600, color:'#64748b', marginBottom:5 }}>Action type</div>
            <select value={action} onChange={e=>{setAction(e.target.value);setPage(1)}}
              style={{ ...iS, width:'100%' }} onFocus={iF} onBlur={iB}>
              <option value="">All actions</option>
              {actions.map(a=>(<option key={a.action} value={a.action}>{a.label} ({a.count})</option>))}
            </select>
          </div>
          <div>
            <div style={{ fontSize:11, fontWeight:600, color:'#64748b', marginBottom:5 }}>From date</div>
            <input type="date" value={from} onChange={e=>{setFrom(e.target.value);setPage(1)}} style={iS} onFocus={iF} onBlur={iB}/>
          </div>
          <div>
            <div style={{ fontSize:11, fontWeight:600, color:'#64748b', marginBottom:5 }}>To date</div>
            <input type="date" value={to} onChange={e=>{setTo(e.target.value);setPage(1)}} style={iS} onFocus={iF} onBlur={iB}/>
          </div>
          {(search||action||from||to) && (
            <button onClick={resetFilters}
              style={{ padding:'9px 16px', borderRadius:8, background:'#fee2e2', color:'#b91c1c', border:'none', cursor:'pointer', fontSize:13, fontWeight:500, alignSelf:'flex-end' }}>
              ✕ Clear
            </button>
          )}
        </div>
      </div>

      {/* ── Log table ─────────────────────────────────────────────────────── */}
      <div style={{ background:'#fff', borderRadius:16, border:'1px solid #e2e8f0', overflow:'hidden' }}>
        <div style={{ padding:'14px 20px', borderBottom:'1px solid #e2e8f0', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div style={{ fontSize:13, color:'#64748b' }}>{total} records</div>
          <div style={{ fontSize:12, color:'#94a3b8' }}>Showing {((page-1)*LIMIT)+1}–{Math.min(page*LIMIT,total)} of {total}</div>
        </div>
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
          <thead>
            <tr style={{ background:'#f8fafc', borderBottom:'1px solid #e2e8f0' }}>
              {['Date & Time','Action','Entity','Performed by','Role'].map(h=>(
                <th key={h} style={{ textAlign:'left', padding:'11px 16px', fontSize:10, fontWeight:600, color:'#64748b', textTransform:'uppercase', letterSpacing:'0.05em' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading ? Array(8).fill(0).map((_,i)=>(
              <tr key={i} style={{ borderBottom:'1px solid #f1f5f9' }}>
                {Array(5).fill(0).map((_,j)=>(
                  <td key={j} style={{ padding:'13px 16px' }}>
                    <div style={{ height:14, background:'#f1f5f9', borderRadius:4, animation:'pulse 1.5s ease infinite' }}/>
                  </td>
                ))}
              </tr>
            )) : logs.length===0 ? (
              <tr><td colSpan={5} style={{ textAlign:'center', padding:52, color:'#94a3b8' }}>
                <div style={{ fontSize:36, marginBottom:10 }}>📋</div>
                <div style={{ fontWeight:600, color:'#64748b' }}>No logs found</div>
                <div style={{ fontSize:13, marginTop:4 }}>Try adjusting your filters</div>
              </td></tr>
            ) : logs.map(log=>{
              const rc = ROLE_COLORS[log.user?.role] || ROLE_COLORS.MEMBER
              return (
                <tr key={log.id} style={{ borderBottom:'1px solid #f1f5f9', transition:'background 0.1s' }}
                  onMouseEnter={e=>(e.currentTarget as HTMLElement).style.background='#f8fafc'}
                  onMouseLeave={e=>(e.currentTarget as HTMLElement).style.background='transparent'}>
                  <td style={{ padding:'13px 16px', whiteSpace:'nowrap' }}>
                    <div style={{ fontSize:12, fontWeight:500, color:'#374151' }}>
                      {new Date(log.createdAt).toLocaleDateString('en-KE',{day:'numeric',month:'short',year:'numeric'})}
                    </div>
                    <div style={{ fontSize:11, color:'#94a3b8', marginTop:2 }}>
                      {new Date(log.createdAt).toLocaleTimeString('en-KE',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}
                    </div>
                  </td>
                  <td style={{ padding:'13px 16px' }}>
                    <div style={{ display:'inline-flex', alignItems:'center', gap:6 }}>
                      <div style={{ width:7, height:7, borderRadius:'50%', background:log.color, flexShrink:0 }}/>
                      <span style={{ fontWeight:500, color:'#0f2040', fontSize:13 }}>{log.label}</span>
                    </div>
                    {log.entityId && <div style={{ fontSize:11, color:'#94a3b8', marginTop:2, fontFamily:'monospace' }}>ID: {log.entityId.slice(0,8)}...</div>}
                  </td>
                  <td style={{ padding:'13px 16px' }}>
                    <span style={{ fontSize:11, background:'#f1f5f9', color:'#475569', padding:'2px 8px', borderRadius:6, fontWeight:500 }}>{log.entity}</span>
                  </td>
                  <td style={{ padding:'13px 16px' }}>
                    <div style={{ fontWeight:500, fontSize:13, color:'#0f2040' }}>{log.user?.fullName||'—'}</div>
                    <div style={{ fontSize:11, color:'#94a3b8', marginTop:1 }}>{log.user?.memberNumber||log.user?.email||'—'}</div>
                  </td>
                  <td style={{ padding:'13px 16px' }}>
                    <span style={{ fontSize:11, fontWeight:600, padding:'2px 9px', borderRadius:99, background:rc.bg, color:rc.color }}>
                      {(log.user?.role||'').replace('_',' ')}
                    </span>
                  </td>
                </tr>
              )
            })}</tbody>
        </table>
        {totalPages>1 && (
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'14px 20px', borderTop:'1px solid #e2e8f0' }}>
            <span style={{ fontSize:13, color:'#94a3b8' }}>Page {page} of {totalPages} · {total} records</span>
            <div style={{ display:'flex', gap:8 }}>
              <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1}
                style={{ padding:'7px 14px', borderRadius:8, border:'1px solid #e2e8f0', background:'#fff', fontSize:13, cursor:page===1?'not-allowed':'pointer', opacity:page===1?.5:1 }}>← Prev</button>
              {Array.from({length:Math.min(5,totalPages)},(_, i)=>{
                const p=Math.min(Math.max(page-2,1)+i,totalPages)
                return <button key={p} onClick={()=>setPage(p)}
                  style={{ padding:'7px 12px', borderRadius:8, border:'1px solid #e2e8f0', background:p===page?'#0f2040':'#fff', color:p===page?'#fff':'#475569', fontSize:13, cursor:'pointer', fontWeight:p===page?600:400 }}>{p}</button>
              })}
              <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={page===totalPages}
                style={{ padding:'7px 14px', borderRadius:8, border:'1px solid #e2e8f0', background:'#fff', fontSize:13, cursor:page===totalPages?'not-allowed':'pointer', opacity:page===totalPages?.5:1 }}>Next →</button>
            </div>
          </div>
        )}
      </div>

      <style>{`
        @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.5} }
        @keyframes spin  { to{transform:rotate(360deg)} }
      `}</style>
    </div>
  )
}
